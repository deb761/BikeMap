...
namespace GMap.NET
{
   ...
   using Thread=OpenNETCF.Threading.Thread2;
    ...
    public class GMaps : Singleton<GMaps>
    {
        ...
        public string VersionGoogleSatelliteKorea = "57";
      ...
      public int RetryLoadTile = 1;
        ...
        /// </summary>
        public readonly int MaxZoom = 22;
        ...
        private DateTime _LastWebResponseTime;
      ...
      public bool CorrectGoogleVersions = false;
        ...
        public void EnqueueCacheTask(CacheItemQueue task)
        {
            Debug.WriteLine(string.Format("{0:dd.MM.yyyy HH:mm:ss.ffff}, Thread={1} GMaps.EnqueueCacheTask is adding task {2}", DateTime.Now, Thread.CurrentThread.ManagedThreadId, task));
            lock (tileCacheQueue)
            {
                if (!tileCacheQueue.Contains(task))
                {
                    ...
                    CheckStartCacheEngine();
                }
            }
        }

        public void CheckStartCacheEngine()
        {
            if (CacheEngine != null && CacheEngine.IsAlive)
            {
                ...
                CacheEngine.ThreadState == System.Threading.ThreadState.Aborted ||
                CacheEngine.ThreadState == System.Threading.ThreadState.Stopped ||
        ...
        void CacheEngineLoop()
        {
            ...
            Debug.WriteLine(string.Format("CacheEngine: stop, TileLoadQueue={0}", Core.Instance.tileLoadQueue.Count));
            //sometimes not all tiles are downloaded!
            if (Core.Instance.tileLoadQueue.Count > 0)
            {
                System.Diagnostics.Debug.WriteLine(string.Format("{0} GMaps.CacheEngineLoop - Restarting Cache Engine", DateTime.Now));
                //for (int i = 0; i < Core.Instance.tileLoadQueue.Count; i++)
                //    Core.Instance.ProcessLoadTask(null);
                //CheckStartCacheEngine();
                Core.Instance.StartProcessLoadTasks();
            }
        }
                ...
                case MapType.GoogleMapKorea:
                    {
                        ...
                        string ret = string.Format("http://{0}{1}.gmaptiles.co.kr/{2}/v={3}&hl={4}&x={5}{6}&y={7}&z={8}&s={9}", server, GetServerNum(pos, 4), request, VersionGoogleMapKorea, language, pos.X, sec1, pos.Y, zoom, sec2);
                        return ret;
                    }
        ...
        /// <param name="pos"></param>
        /// <returns></returns>
        public int GetServerNum(Point pos, int max)
        {
                ...
                if (ret == null)
                {
                        ...
                        if (Cache.Instance.ImageCache != null)
                        {
                            ...
                            if (ret != null)
                            {
                                if (UseMemoryCache)
                                {
                                    //if (ret.Data == null)
                                    //{
                                    //    AddTileToMemoryCache(new RawTile(type, pos, zoom), ret.im);
                                    //}
                                    //else
                                    //{
                    ...
                    if (Mode != AccessMode.CacheOnly)
                    {
                     ...
                     request.Proxy = GlobalProxySelection.GetEmptyWebProxy();
                        ...
                        System.Diagnostics.Debug.WriteLine(string.Format("{0} Thread={1} GMaps.GetImageFrom, pos={2}: Starting GetResponse.", DateTime.Now, Thread.CurrentThread.ManagedThreadId, pos));
                        while (DateTime.Now < _LastWebResponseTime)
                        {
                            System.Threading.Thread.Sleep(5);
                        }
                        _LastWebResponseTime = DateTime.Now.AddSeconds(3);

                        using (HttpWebResponse response = request.GetResponse() as HttpWebResponse)
                        {
                            System.Diagnostics.Debug.WriteLine(string.Format("{0} Thread={1} GMaps.GetImageFrom, pos={2}: GetResponse OK.", DateTime.Now, Thread.CurrentThread.ManagedThreadId, pos));

                            System.Diagnostics.Debug.WriteLine(string.Format("{0} Thread={1} GMaps.GetImageFrom, pos={2}: Starting GetResponseStream.", DateTime.Now, Thread.CurrentThread.ManagedThreadId, pos));
                            MemoryStream responseStream = Stuff.CopyStream(response.GetResponseStream(), false);
                            {
                                System.Diagnostics.Debug.WriteLine(string.Format("{0} Thread={1} GMaps.GetImageFrom, pos={2}: GetResponseStream OK.", DateTime.Now, Thread.CurrentThread.ManagedThreadId, pos));
                                ...
                                if (GMaps.Instance.ImageProxy != null)
                                {
                                    ...
                                    if (ret != null)
                                    {
                                        ...
                                        if (Mode != AccessMode.ServerOnly)
                                        {
                                            CacheItemQueue task = new CacheItemQueue(type, pos, zoom, responseStream, CacheUsage.Both);
                                            EnqueueCacheTask(task);
                                        }
                                    }
                                }
                            }
                            response.Close();
                            System.Diagnostics.Debug.WriteLine(string.Format("{0} Thread={1} GMaps.GetImageFrom, pos={2}: GetResponseStream DONE.", DateTime.Now, Thread.CurrentThread.ManagedThreadId, pos));
                        }
                        _LastWebResponseTime = DateTime.Now;
                    }
                }
            }
            catch (Exception ex)
